...
# remember to comment out the pdb.set_trace() lines when you are done.
app = Flask(__name__)
...
def signup():
    ...
    if form.validate_on_submit():
        try:
            ...
            # import pdb; pdb.set_trace() #breakpoint here to explore the user obj before commit
            db.session.commit()
...
def login():
    ...
    if form.validate_on_submit():
        user = User.authenticate(form.username.data,
                                 form.password.data)
        # import pdb; pdb.set_trace() #breakpoint to explore the user object after authentication
...
def messages_add():
    ...
    if form.validate_on_submit():
        ...
        # import pdb; pdb.set_trace() # debugging before commiting message
        db.session.commit()
...
def show_likes(user_id):
    ...
    liked_messages = [like.message for like in likes]
...
def toggle_like(message_id):
    ...
    if like:
        #if like exist remove it.
        db.session.delete(like)
        flash("Message unliked.", "success")
    else:
        #if like doesn't exist add a new like
        if not Message.query.get(message_id).user_id == g.user.id:
    ...
    try:
        ...
        db.session.rollback()
        flash("Could not process your like.", "error")
